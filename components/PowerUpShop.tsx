...
/** Halaman Toko Power-Up: tukar Coin dengan stok, dipakai belakangan lewat tombol di papan permainan. */
export default function PowerUpShop() {
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-[46rem] flex-col gap-6 px-4 py-8 sm:px-6">
      ...
      <div className="rise flex items-center justify-between gap-4 rounded-lg border border-[var(--line)] bg-[var(--card)] px-5 py-3.5" style={{ "--step": 1 } as React.CSSProperties}>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {POWER_UP_KINDS.map((kind, i) => {
          ...
          return (
            <div
              key={kind}
              className="rise mode-card flex h-full flex-col rounded-xl border border-[var(--line)] bg-[var(--card)] p-5"
              style={{ "--accent": accent, "--step": i + 2 } as React.CSSProperties}
            >
              <span aria-hidden="true" style={{ color: accent }} className="grid size-11 place-items-center rounded-full border border-[var(--line)]">
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
