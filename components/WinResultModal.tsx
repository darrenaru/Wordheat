...
/**
 * Muncul sekali begitu ada pemain yang berhasil menebak kata rahasianya --
 * pemain lain yang sedang tidak melihat papan langsung tahu siapa yang
 * menang, apa katanya, dan bagaimana performa semua orang, tanpa harus
 * membaca satu baris teks kecil di tengah papan.
 */
export default function WinResultModal({
  ...
  // Fokus awal & penguncian scroll body cuma boleh terjadi sekali saat modal
  // benar-benar mount -- lihat catatan yang sama di ChatModal/RoomChatModal:
  // `onClose` yang dikirim RoomBoard adalah fungsi anonim baru di tiap
  // render (dipicu update SSE room), jadi kalau ini digabung ke efek yang
  // bergantung pada `onClose`, tiap update room akan memanggil ulang
  // dialogRef.current?.focus() tanpa alasan.
  useEffect(() => {
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[13px] text-[var(--muted)]">
              <strong className="text-[var(--fg)]">{winnerName}</strong> menemukannya dalam{" "}
              <strong className="text-[var(--fg)]">{winnerGuessCount}</strong> tebakan.
            </p>
          </div>
          <button
            type="button"
        ...
        <ul className="flex flex-col gap-1">
          {ranked.map((player) => {
            ...
            return (
              <li
                key={player.id}
                className={`flex items-center justify-between gap-3 rounded-lg border px-3 py-2 text-[14px] ${
                  won ? "border-gold/50 bg-gold/10" : "border-[var(--line)]"
                }`}
              >
